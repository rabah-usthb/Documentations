...
\begin{prettyBox}{Synonymes}{myblue}
    \begin{itemize}
        ...
        \item On ne peut pas acceder a un synonym d'une autre instance. 
    \end{itemize}
\end{prettyBox}
...
\section*{Requête Select}

\begin{codeInput}{sql}{Select Dans ORCL Et ORCL2}{code01}{41}
-- ORCL :
-- Affiche les num de fournisseurs pour les commandes Du Site ORCL
-- De produit num 1 et de projet num 9
SELECT DISTINCT numf FROM Commande
WHERE nump = 1 AND numj = 9;

-- Affiche les numero,nom,ville des fournisseurs D'oran de la vue
SELECT numf, nomf, ville FROM vw_Fournisseur WHERE ville = 'ORAN';

-- Affiche les noms des fournisseurs du Site ORCL 
select nomf from fournisseur;

-- ORCL2 :
-- Affiche les produit du Site ORCL qui a le poid minimum
SELECT * FROM Produit1@link_orcl
WHERE poids = (SELECT MIN(poids) FROM Produit1@link_orcl);

-- Affiche les num de fournisseurs des commande Du Site ORCL2
-- Dont le num projet est 10
SELECT DISTINCT numf FROM Commande
WHERE numj = 10;

-- Affiche le Numero de produit des commandes Du site ORCL2
-- Avec la plus grand quantite
SELECT nump FROM Commande
GROUP BY nump
...
\begin{figure}[H] 
  \centering 
  \includegraphics[width=0.65\textwidth]{select1.png} 
\end{figure}


\begin{figure}[H] 
  \centering 
  \includegraphics[width=0.65\textwidth]{select2.png} 
\end{figure}



\newpage
\section*{Test Trigger Insertion}
\begin{codeInput}{sql}{Vue Et Trigger}{code01}{41}
