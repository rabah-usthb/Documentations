...
plt.plot(N, experimental_time_2, 's-', label='Experimental Second Solution (Time x 10^-3)', color='red')

plt.xlim(left=min(N))  # Start x-axis at the minimum N value
plt.ylim(bottom=min(min(experimental_time_1), min(experimental_time_2)))  # Start y-axis at the minimum time value


# Labels and title
plt.xlabel('N (Data Size)')
plt.ylabel('Time (seconds)')
plt.title('Solution 1 vs Solution 2')
plt.legend()
