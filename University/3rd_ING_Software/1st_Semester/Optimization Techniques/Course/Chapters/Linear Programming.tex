...
\begin{prettyBox}{Objective Function}{box}
\[
f(x_i) = Z = 3\times(160x_1 + 100x_2 + 200x_3 + 75x_3) + 7\times(30x_1 + 35x_2 + 60x_3 + 80x_4)\\
...
to B$_{j}$ cost from A$_{i}$ To B$_{j}$, the goal is to find a schedule to minimize the cost 
...
where \(t_1 \geq 0\) is the artificial variable added to the equation.\\[0.25cm]

If the constraint is by default an equality we add an artificial variable:
\[
x_1 + x_2 = 10 \quad \Longrightarrow \quad x_1 + x_2 + t_1 = 10
\]
\end{prettyBox}
...
\begin{itemize}
    ...
    \item We can prune a node if there is another finished node at the same level with the same \(Z\) value, as branching further would only decrease \(Z\) (for a maximization problem).
\end{itemize}
\end{prettyBox}
...
\subsubsection{Changing the Coefficient of the Objective Function}

\begin{prettyBox}{Range Of Optimality}{box}
When changing the value of a coefficient in the objective function, there is a range of values for which the optimal solution remains unaffected.
\end{prettyBox}

\vspace{0.25cm}
\begin{prettyBox}{Binding \& Non-Binding Constraints}{red}
A constraint is considered \textbf{binding} if substituting the optimal solution into the constraint results in \(\text{LHS} = \text{RHS}\). Otherwise, it is \textbf{non-binding}.
\end{prettyBox}

\vspace{0.25cm}
\subsubsection{Changing the RHS of a Constraint}
\begin{prettyBox}{Impact on \(Z\)}{box}
There are two cases:
\begin{itemize}
    \item \textbf{Binding (Saturated):} Changing the RHS of a binding constraint will affect \(Z\), and the shadow price is calculated as:
    \[
    \text{Shadow Price} = Z_{\text{new}} - Z_{\text{old}} \neq 0
    \]
    \item \textbf{Non-Binding:} Changing the RHS of a non-binding constraint will not affect \(Z\), and the shadow price is \(0\).
\end{itemize}
\end{prettyBox}
